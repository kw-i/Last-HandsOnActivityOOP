...
public class InvoiceDatabase{
    ...
    public boolean addInfo(String invno, String customer, int amount, int payment) {
        ...
        String checkQuery = "SELECT payment FROM receivable WHERE invno = ?";
        String deleteQuery = "DELETE FROM receivable WHERE invno = ?";
 
        try (Connection conn = DriverManager.getConnection(url, dbUser, dbPassword);
            PreparedStatement checkPst = conn.prepareStatement(checkQuery)) {
 
            checkPst.setString(1, invno);
            ResultSet rs = checkPst.executeQuery();
 
            if (rs.next()) {
                int payment = rs.getInt("payment");
                if (payment > 0) {
                    System.out.println("Cannot delete invoice: a payment has already been made.");
                    return false;
                }
            }
 
            try (PreparedStatement deletePst = conn.prepareStatement(deleteQuery)) {
                deletePst.setString(1, invno);
